...
export type CityActionType = 'farm' | 'patrol' | 'recruit' | 'search';
...
function koreanSubject(name: string): string {
...
function campaignId(seed: string): string {
  return `tc-${seed.replace(/[^a-z0-9]/gi, '').slice(-10)}-${Date.now().toString(36).slice(-5)}`;
}

function addChronicle(
  state: GameState,
  category: GameState['chronicle'][number]['category'],
  title: string,
  body: string,
  importance: 1 | 2 | 3 | 4 | 5 = 1,
): void {
...
function initialOfficerStates(): Record<OfficerId, OfficerState> {
  const states = {} as Record<OfficerId, OfficerState>;
  const active = new Set<OfficerId>(['cao_cao', 'xiahou_dun', 'dian_wei', 'liu_bei', 'guan_yu', 'zhang_fei']);
  for (const officer of Object.values(OFFICERS) as OfficerDefinition[]) {
    states[officer.id] = {
      id: officer.id,
...
function initialCities(): Record<CityId, CityState> {
  return {
    xuchang: {
...
function initialFactions(): Record<'cao' | 'liu', FactionState> {
  return {
...
export function createNewGame(playerFactionId: 'cao' | 'liu', seed = makeSeed()): GameState {
  ...
  addChronicle(
    ...
    'system',
    '새 연대기가 시작되다',
    `${koreanSubject(FACTIONS[playerFactionId].name)} ${CITY_DEFINITIONS[FACTIONS[playerFactionId].capitalId].name}에서 첫 계절을 맞았습니다. 진류를 거쳐 상대 수도를 점령하십시오.`,
    4,
  );
  return state;
}

export function factionCities(state: GameState, factionId: Exclude<FactionId, 'neutral'>): CityState[] {
  return (Object.values(state.cities) as CityState[]).filter((city) => city.ownerId === factionId);
}

export function activeOfficers(
  state: GameState,
  factionId: Exclude<FactionId, 'neutral'>,
  cityId?: CityId,
): OfficerState[] {
  return (Object.values(state.officers) as OfficerState[])
...
export function candidatesForFaction(state: GameState, factionId: Exclude<FactionId, 'neutral'>): OfficerState[] {
  return (Object.values(state.officers) as OfficerState[])
...
function bestOfficerAtCity(
  state: GameState,
  factionId: Exclude<FactionId, 'neutral'>,
  cityId: CityId,
  stat: keyof OfficerDefinition['stats'],
): OfficerState | undefined {
...
function ensurePlayerAction(state: GameState, cityId: CityId): string | null {
  if (state.status !== 'playing') return '이미 연대기가 끝났습니다.';
  if (state.pendingEventId) return '먼저 계절 사건을 결정해 주세요.';
  if (state.actionPoints <= 0) return '이번 계절의 행동점을 모두 사용했습니다.';
  if (state.cities[cityId].ownerId !== state.playerFactionId) return '우리 세력의 도시에서만 실행할 수 있습니다.';
  return null;
}

export function cityActionPreviews(state: GameState, cityId: CityId): ActionPreview[] {
...
export function performCityAction(state: GameState, cityId: CityId, action: CityActionType): ActionResult {
  ...
  if (!preview?.enabled) return fail(state, preview?.reason ?? '지금은 실행할 수 없습니다.');
  ...
  if (action === 'farm') {
    ...
    if (officer) { officer.fatigue = clamp(officer.fatigue + 3, 0, 100); officer.merit += 2; }
  ...
  if (action === 'recruit') {
    ...
    if (officer) { officer.fatigue = clamp(officer.fatigue + 5, 0, 100); officer.merit += 2; }
  ...
  if (action === 'search') {
    faction.gold -= 50;
    const hidden = (Object.values(next.officers) as OfficerState[])
...
export function recruitChance(state: GameState, officerId: OfficerId, factionId = state.playerFactionId): number {
...
export function recruitCandidate(state: GameState, officerId: OfficerId): ActionResult {
  if (state.status !== 'playing') return fail(state, '이미 연대기가 끝났습니다.');
  if (state.pendingEventId) return fail(state, '먼저 계절 사건을 결정해 주세요.');
  if (state.actionPoints <= 0) return fail(state, '행동점이 없습니다.');
  const candidate = state.officers[officerId];
  ...
  const faction = state.factions[state.playerFactionId];
  if (faction.gold < 120) return fail(state, '등용 예물에 필요한 금 120이 부족합니다.');
...
function bestTroopType(officerId: OfficerId): TroopType {
...
function battleValidation(state: GameState, attackerFactionId: 'cao' | 'liu', draft: BattleDraft, usesPlayerAp: boolean): string | null {
  if (state.status !== 'playing') return '이미 연대기가 끝났습니다.';
  if (usesPlayerAp && state.pendingEventId) return '먼저 계절 사건을 결정해 주세요.';
  if (usesPlayerAp && state.actionPoints <= 0) return '공격 명령에 필요한 행동점이 없습니다.';
  const source = state.cities[draft.sourceCityId];
  const target = state.cities[draft.targetCityId];
  if (source.ownerId !== attackerFactionId) return '출발 도시가 공격 세력의 소유가 아닙니다.';
  if (target.ownerId === attackerFactionId) return '우리 도시를 공격할 수 없습니다.';
  if (!CITY_DEFINITIONS[draft.sourceCityId].neighbors.includes(draft.targetCityId)) return '두 도시는 연결되어 있지 않습니다.';
  if (draft.lines.length !== 3 || new Set(draft.lines.map((line) => line.officerId)).size !== 3) return '서로 다른 장수 3명을 좌·중·우에 배치해야 합니다.';
  if (draft.committedTroops < 2400) return '최소 2,400명을 투입해야 합니다.';
  if (source.troops - draft.committedTroops < 800) return '출발 도시에 최소 800명의 수비대를 남겨야 합니다.';
  const foodCost = battleFoodCost(draft.committedTroops);
  if (state.factions[attackerFactionId].food < foodCost) return `원정 군량 ${foodCost}이 부족합니다.`;
  for (const line of draft.lines) {
    ...
    if (officer.fatigue >= 90) return `${OFFICERS[line.officerId].name}의 피로가 너무 높습니다.`;
  }
  return null;
}

export function battleFoodCost(committedTroops: number): number {
  return Math.max(160, Math.round(committedTroops / 12));
}

function retreatDefenders(state: GameState, defenderFactionId: FactionId, targetCityId: CityId): void {
  if (defenderFactionId === 'neutral') return;
  const capitalId = FACTIONS[defenderFactionId].capitalId;
  for (const officer of Object.values(state.officers) as OfficerState[]) {
    if (officer.factionId !== defenderFactionId || officer.status !== 'active' || officer.cityId !== targetCityId) continue;
...
function applyBattleOutcome(state: GameState, result: BattleResult): void {
  ...
  if (result.attackerWon) {
    ...
    addChronicle(
      state,
      'battle',
      `${CITY_DEFINITIONS[result.targetCityId].name}을 점령하다`,
      `${koreanSubject(result.standout)} 가장 큰 공을 세웠습니다. 공격군 손실 ${result.attackerLosses.toLocaleString()}, 수비군 손실 ${result.defenderLosses.toLocaleString()}.`,
      result.defenderFactionId === 'neutral' ? 3 : 4,
    );
    ...
    if (result.defenderFactionId !== 'neutral' && FACTIONS[result.defenderFactionId].capitalId === result.targetCityId) {
      ...
      addChronicle(
        state,
        'battle',
        state.status === 'victory' ? '상대 수도의 깃발이 내려가다' : '우리 수도를 잃다',
        state.status === 'victory'
          ? `${koreanSubject(FACTIONS[state.playerFactionId].name)} 3도시 수직 슬라이스의 승리를 완성했습니다.`
          : `${FACTIONS[state.enemyFactionId].name}의 공격으로 첫 연대기가 막을 내렸습니다.`,
        5,
      );
    }
  } else {
    ...
    addChronicle(
      state,
      'battle',
      `${CITY_DEFINITIONS[result.targetCityId].name}에서 물러나다`,
      `${koreanSubject(result.standout)} 가장 치열하게 싸웠습니다. ${result.factors[0] ?? '수비군의 대열이 더 오래 버텼습니다.'}`,
      3,
    );
  }
...
function executeBattleForFaction(
  state: GameState,
  attackerFactionId: 'cao' | 'liu',
  draft: BattleDraft,
  usesPlayerAp: boolean,
): ActionResult {
  ...
  if (usesPlayerAp) next.actionPoints -= 1;
...
export function attackCity(state: GameState, draft: BattleDraft): ActionResult {
  return executeBattleForFaction(state, state.playerFactionId, draft, true);
}

function aiSearchOrRecruit(state: GameState, factionId: 'cao' | 'liu'): boolean {
  ...
  if (faction.gold < 50) return false;
  const hidden = (Object.values(state.officers) as OfficerState[])
...
function aiDomestic(state: GameState, factionId: 'cao' | 'liu'): boolean {
...
function aiAttackDraft(state: GameState, factionId: 'cao' | 'liu'): BattleDraft | null {
  const options: { source: CityState; target: CityState; score: number }[] = [];
  for (const source of factionCities(state, factionId)) {
    ...
    if (officers.length < 3 || source.troops < 4200) continue;
    for (const targetId of CITY_DEFINITIONS[source.id].neighbors) {
      const target = state.cities[targetId];
      if (target.ownerId === factionId) continue;
      if (target.ownerId === 'neutral' && state.turn < 3) continue;
      if (target.ownerId !== 'neutral' && state.turn < 2) continue;
      ...
      if (ratio >= (target.ownerId === 'neutral' ? 1.18 : 1.05)) options.push({ source, target, score });
    }
  }
  ...
  if (state.factions[factionId].food < battleFoodCost(committedTroops)) return null;
  return {
...
function incomeForCity(state: GameState, city: CityState): { gold: number; food: number; maintenance: number } {
  if (city.ownerId === 'neutral') return { gold: 0, food: 0, maintenance: 0 };
  ...
  if (governor?.id && OFFICERS[governor.id].trait === 'steward') gold = Math.round(gold * 1.18);
  if (governor?.id && OFFICERS[governor.id].trait === 'supply') food = Math.round(food * 1.2);
  const maintenance = Math.round(city.troops / 26);
  return { gold, food, maintenance };
}

function settleSeason(state: GameState): void {
  for (const city of Object.values(state.cities) as CityState[]) {
    if (city.ownerId === 'neutral') continue;
    ...
    if (city.order < 45) faction.gold = Math.max(0, faction.gold - 35);
  ...
  for (const officer of Object.values(state.officers) as OfficerState[]) {
    ...
    if (officer.factionId && state.factions[officer.factionId].gold < 80) officer.loyalty = clamp(officer.loyalty - 2, 0, 100);
  }
}

function chooseNextEvent(state: GameState): StoryEventDefinition | undefined {
...
function setTurnAndSeason(state: GameState): void {
  state.turn += 1;
...
function resolveTurnLimit(state: GameState): void {
  if (state.turn <= VERTICAL_MAX_TURN || state.status !== 'playing') return;
  ...
  addChronicle(
    ...
    'system',
    '24턴 판정',
    `도시와 명성을 합산한 결과 ${koreanSubject(state.status === 'victory' ? FACTIONS[state.playerFactionId].name : FACTIONS[state.enemyFactionId].name)} 앞섰습니다.`,
    5,
  );
}
...
export function endTurn(state: GameState): ActionResult {
  ...
  if (next.status !== 'playing') return { ok: true, state: next, message: '적의 공격으로 연대기가 끝났습니다.' };
  settleSeason(next);
  ...
  if (next.status === 'playing') {
    ...
    if (event) {
      ...
      if (!next.usedEventIds.includes(event.id)) next.usedEventIds.push(event.id);
    }
  }
...
function effectCityId(state: GameState, effect: Extract<Effect, { type: 'city' }>): CityId {
...
function applyEffect(state: GameState, effect: Effect): void {
  ...
  if (effect.type === 'city') {
    ...
    city[effect.stat] = clamp(city[effect.stat] + effect.amount, 0, max) as never;
    return;
  }
  if (effect.type === 'loyalty') {
    const officers = activeOfficers(state, state.playerFactionId);
    if (effect.target === 'all') officers.forEach((officer) => { officer.loyalty = clamp(officer.loyalty + effect.amount, 0, 100); });
    else if (effect.target === 'lowest') {
      const target = officers.sort((a, b) => a.loyalty - b.loyalty)[0];
      if (target) target.loyalty = clamp(target.loyalty + effect.amount, 0, 100);
    } else {
  ...
  if (effect.type === 'fatigue') {
    const officers = activeOfficers(state, state.playerFactionId);
    if (effect.target === 'all') officers.forEach((officer) => { officer.fatigue = clamp(officer.fatigue + effect.amount, 0, 100); });
    else if (effect.target === 'highest') {
      const target = officers.sort((a, b) => b.fatigue - a.fatigue)[0];
      if (target) target.fatigue = clamp(target.fatigue + effect.amount, 0, 100);
    } else {
      const target = state.officers[effect.target];
      if (target) target.fatigue = clamp(target.fatigue + effect.amount, 0, 100);
    }
  ...
  if (effect.type === 'contact') {
    ...
    if (target) {
      ...
      if (target.contact > 0) target.status = 'candidate';
    }
  }
}

export function pendingEvent(state: GameState): StoryEventDefinition | null {
  if (!state.pendingEventId) return null;
  return STORY_EVENTS.find((event) => event.id === state.pendingEventId) ?? null;
}

export function resolveEventChoice(state: GameState, choiceId: string): ActionResult {
  const event = pendingEvent(state);
  if (!event) return fail(state, '결정할 사건이 없습니다.');
  const choice = event.choices.find((item) => item.id === choiceId);
  if (!choice) return fail(state, '선택지를 찾을 수 없습니다.');
  const next = cloneGame(state);
...
export function connectedTargets(state: GameState, sourceCityId: CityId, factionId = state.playerFactionId): CityState[] {
  if (state.cities[sourceCityId].ownerId !== factionId) return [];
  return CITY_DEFINITIONS[sourceCityId].neighbors
    .map((id) => state.cities[id])
    .filter((city) => city.ownerId !== factionId);
}

export function assignGovernor(state: GameState, cityId: CityId, officerId: OfficerId): ActionResult {
  const error = ensurePlayerAction(state, cityId);
  if (error) return fail(state, error);
  const officer = state.officers[officerId];
...
export function invariantErrors(state: GameState): string[] {
  const errors: string[] = [];
  if (state.actionPoints < 0 || state.actionPoints > 3) errors.push('행동점 범위 오류');
  for (const faction of Object.values(state.factions) as FactionState[]) {
    if (faction.gold < 0 || faction.food < 0 || faction.fame < 0) errors.push(`${faction.id} 자원 음수`);
  }
  for (const city of Object.values(state.cities) as CityState[]) {
    if (!['cao', 'liu', 'neutral'].includes(city.ownerId)) errors.push(`${city.id} 소유자 오류`);
    if (city.troops < 0 || city.food < 0) errors.push(`${city.id} 병력/군량 음수`);
    if (city.order < 0 || city.order > 100 || city.wall < 0 || city.wall > 100) errors.push(`${city.id} 도시 수치 범위 오류`);
  }
  for (const officer of Object.values(state.officers) as OfficerState[]) {
    if (officer.loyalty < 0 || officer.loyalty > 100 || officer.fatigue < 0 || officer.fatigue > 100) errors.push(`${officer.id} 상태 범위 오류`);
    if (officer.status === 'active' && !officer.factionId) errors.push(`${officer.id} 활성 장수 소속 없음`);
    if (!state.cities[officer.cityId]) errors.push(`${officer.id} 도시 참조 오류`);
  }
  return errors;
}

export function campaignScore(state: GameState, factionId: 'cao' | 'liu'): number {
...
export function recommendedHint(state: GameState): string {
  if (state.pendingEventId) return '새 계절의 사건을 먼저 결정하세요.';
  ...
  if (state.actionPoints === 3 && state.turn === 1) return `${CITY_DEFINITIONS[FACTIONS[state.playerFactionId].capitalId].name}에서 개간이나 순찰로 첫 행동을 시작해 보세요.`;
  if (candidatesForFaction(state, state.playerFactionId).length) return '조정에 접촉한 인재가 기다립니다. 성공 가능성과 역할을 확인해 보세요.';
  if (border && border.troops < 5200) return `${CITY_DEFINITIONS[border.id].name}에서 징병해 최소 5,200명 이상을 준비하는 편이 안전합니다.`;
  if (border && activeOfficers(state, state.playerFactionId, border.id).length >= 3 && connectedTargets(state, border.id).length) return '군단 화면에서 좌·중·우 병종과 진형을 조정해 공격을 준비할 수 있습니다.';
  if (state.actionPoints === 0) return '행동점을 모두 사용했습니다. 계절 종료로 적의 움직임과 수입을 해결하세요.';
